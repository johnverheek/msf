...
1. Compute the xxHash3-64 digest with seed 0 of buffered bytes 0–39 and compare to the u64 at offsets 40–47
1. Inspect the major version field at offsets 4–5
...
u32     Entity count
  [per entity]
  f64   Position X (relative to anchor)
  f64   Position Y (relative to anchor)
  f64   Position Z (relative to anchor)
  f32   Yaw
  f32   Pitch
  str   Entity type (e.g. "minecraft:armor_stand")
  u16   NBT payload length
  u8[]  NBT payload (binary NBT, excludes position, rotation, and type)
```
...
**Entity positions are anchor-relative offsets.** Entity positions MUST be stored as f64 offsets relative to the schematic anchor point declared in the placement metadata (see Section 10), consistent with the coordinate system defined in Section 7.7. Writers MUST subtract the anchor world position from the entity world position before writing each position field. Readers MUST add the placement anchor world position to each stored position field before spawning the entity. Writers MUST NOT store absolute world coordinates in the entity position fields.

**Rotation MUST NOT be included in the NBT payload.** Yaw and pitch are captured in typed fields and MUST NOT be present in the NBT payload. Writers MUST strip rotation data from entity NBT before writing. The NBT payload exclusion covers position data, rotation data, and entity type — all three are present as typed fields in the layout.
